...
func NewStorage(root string, opts ...OptionFunc) (*BlobStorage, error) {
	// Copy default options to avoid mutating the global default
	// Why: defaultOpts is a package-level variable that should remain constant
	options := &Options{
...
func (bs *BlobStorage) Put(ctx context.Context, key string, r io.Reader) error {
	blob, err := bs.NewBlob()
	if err != nil {
		return err
	}
...
// writeAndHash writes data from reader to file while computing SHA-256 hash
// and detecting content type. Returns contentType, hash, size, and error.
func (bs *BlobStorage) writeAndHash(w io.Writer, r io.Reader) (contentType, hash string, size int64, err error) {
	// Read first 512 bytes for content type detection
	// Why 512: http.DetectContentType requires up to 512 bytes to accurately
	// determine the MIME type based on file signatures
	buf := make([]byte, 512)
	n, _ := io.ReadFull(r, buf)
	buf = buf[:n]
	contentType = http.DetectContentType(buf)

	// Hash content while writing
	hasher := sha256.New()
	mw := io.MultiWriter(w, hasher)

	if _, err := mw.Write(buf); err != nil {
		return "", "", 0, fmt.Errorf("writing header to temp file: %w", err)
	}

	written, err := io.Copy(mw, r)
	if err != nil {
		return "", "", 0, fmt.Errorf("copying content to temp file: %w", err)
	}

	size = int64(len(buf)) + written
	hash = hex.EncodeToString(hasher.Sum(nil))

	return contentType, hash, size, nil
}

// writeMeta writes blob metadata to a JSON file.
func (bs *BlobStorage) writeMeta(path string, meta *Meta) error {
...
func (bs *BlobStorage) Get(ctx context.Context, key string) (io.ReadCloser, error) {
...
func (bs *BlobStorage) newTempFile() (*os.File, error) {
	tempPath := filepath.Join(bs.root, tempDirName, newID())
	return os.Create(tempPath)
}

func (bs *BlobStorage) Stat(ctx context.Context, key string) (*Meta, error) {
	if err := bs.validateKey(key); err != nil {
		return nil, err
	}
...
//	    // handle error
//	}
func (bs *BlobStorage) List(ctx context.Context, prefix string) *BlobResult {
...
func (bs *BlobStorage) walkBlobs(ctx context.Context, prefix string, metaChan chan<- *Meta, errChan chan<- error) {
...
func (bs *BlobStorage) createPathFromKey(key string) string {
	shardPath := bs.opts.ShardFunc(key)
	return filepath.Join(bs.root, blobDirname, shardPath)
}

func (bs *BlobStorage) readMeta(path string) (*Meta, error) {
	f, err := os.Open(path)
	if err != nil {
		return nil, err
	}
...
func (bs *BlobStorage) validateKey(key string) error {
	if key == "" {
		return ErrEmptyKey
	}
...
func (bs *BlobStorage) cleanupEmptyDirs(path string) {
	blobsDir := filepath.Join(bs.root, blobDirname)
