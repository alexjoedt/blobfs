...
type Blob struct {
	...
	// Temp file handling
	// Why: temporary file ensures atomic blob creation - either the blob
	// is fully written or it doesn't exist at all
	tmpFile *os.File
	tmpPath string

	// Content tracking
	// Why: SHA-256 is computed incrementally during write to avoid
	// re-reading large files after writing
	hasher hash.Hash
	size   int64

	// Content type detection buffer
	// Why: http.DetectContentType needs first 512 bytes, so we buffer them
	// rather than storing the entire content in memory
	buffer     []byte
	bufferUsed int
...
func (bs *BlobStorage) NewBlob() (*Blob, error) {
	// Create temp file
...
func (b *Blob) CommitAs(key string) error {
	...
	// Write metadata
	// Why: metadata is written before moving the blob to ensure consistency
